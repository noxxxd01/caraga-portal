...
function updateParticipantsSummary() {
  ...
  const officeOrder = Object.keys(CARAGA_PROVINCE_COORDINATES);
  const maleByProvince = officeOrder.map(
    (name) =>
      participantsData.filter((p) => p.province === name && p.sex === 'Male')
        .length,
  );
  const femaleByProvince = officeOrder.map(
    (name) =>
      participantsData.filter((p) => p.province === name && p.sex === 'Female')
        .length,
  );
  recalcParticipantsChart(officeOrder, maleByProvince, femaleByProvince);
}
